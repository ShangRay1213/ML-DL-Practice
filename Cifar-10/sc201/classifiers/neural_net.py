...
class TwoLayerNet(object):
    ...
    def cost(self, X, y=None):
        ...
        dA = dF.dot(W2.T)
        dK = dA*np.where(K>0, 1, 0)
        grads['W1'] = X.T.dot(dK)
        grads['b1'] = np.sum(dK, axis = 0)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            c = np.random.choice(num_train, batch_size, replace=True)
            X_batch = X[c]
            y_batch = y[c]
    ...
    def predict(self, X):
        ...
        ###########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        K = np.dot(X, self.params['W1']) + self.params['b1']
        A = np.maximum(0, K)
        scores = np.dot(A, self.params['W2']) + self.params['b2']
