...
def neural_network(X, Y):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i in range(1, L + 1):
            ...
            d['A' + str(i)] = A
        H = A
        dA = (1/m)*np.sum(H-Y, axis=0, keepdims=True)
        # Backward Pass
        for i in reversed(range(1, L + 1)):
            if i == L:
                dK = dA
            else:
                dK = dA * np.where(d['K' + str(i)] > 0, 1, 0)
            dW = d['A' + str(i - 1)].dot(dK.T) if i > 1 else X.dot(dK.T)
            dB = np.sum(dK, axis=1, keepdims=True)

            dA = weights['W' + str(i)].dot(dK)
            # Update weights and biases
            weights['W' + str(i)] -= ALPHA * dW
            biases['B' + str(i)] -= ALPHA * dB
